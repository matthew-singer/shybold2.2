...
public:
    ...
    double distance(const std::shared_ptr<agent> &p) {
      return pow(pow(p->x - x, 2) + pow(p->y - y, 2) , .5); //+pow(p->x2-x2, 2)  + pow(p->y2-y2, 2) , .5);
    }
    
    bool between_agent(const std::shared_ptr<agent> &p) {
      double tmp2 =  atan2(p->y - y, p->x - x);//, p->x2-x2, p->y2-y2);
        if (tmp2 < 0) {
            tmp2 += 2 * PI;
        }
        ...
        if (p->y - y == 0 && p->x - x == 0 ){//|| p-> x2-x2 == 0 && p->y2-y2 ==0 ) { //on top of each other, auto eat
            return true;
        }
        return false;
    }
    ...
    int lastTime;
    void resetLocation() {
      setPoints(rates(mutate) * sizeX, rates(mutate) * sizeY );//rates(mutate) * sizeX2, rates(mutate) * sizeY2,); //set to random
        alive=true;
    }
