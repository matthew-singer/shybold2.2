...
#endif
    const int input  = 4; //bias node and delta x and delta y and random
    const int input_agents = 1; // number of agents to input into the NN
    const int inputs_foodlookup = 2; //number of food looked at each timestep and input to NN
//Parameters for the predation function
const int inputs_food = 5;



    const double i_s = 0.0;
    const double r_s = 0.02;
    const double c_s = 250.0;
    ...
    const int output = 3;
//const int outputfood = 4;
   const int outputfood = 2;
    const int hiddenLayerSize = 0;
    const int hiddenLayers = 0;
    
//const int geneNN = ((input+input_agents*2+inputs_food)*output)*((hiddenLayerSize+1)*(hiddenLayers+1));
//const int geneNN = ((input+input_agents*2)*output)*((hiddenLayerSize+1)*(hiddenLayers+1)+ inputs_food);
const int geneNN = ((input+input_agents*2)*output)*((hiddenLayerSize+1)*(hiddenLayers+1));
    ...
    const double sizeX2 = 1000;
    const double sizeY2 = 500;
