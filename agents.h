...
public:
    ...
    void setPoints2(double x2_,double y2_) {x2 = x2_; y2 = y2_;}
    ...
    bool valid_agent(const std::shared_ptr<agent> &p) {
        if (p->alive && this->distance(p) < g->getRadius()) {
            if (this->between_agent(p)) {
               if (!input_agent[0]||!input_agent[1]) {
                  return true;
    ...
    bool valid_agent2(const std::shared_ptr<agent> &p) {
      if (p->alive && this->distance(p) < g->getRadius()) {
	if (this->between_agent(p)) {
	  if (!input_agent[0]) {
	    return true;
    ...
    //    std::vector<std::shared_ptr<input_agent[]>outputfood_values;
    ...
    void resetLocation() {
      ...
      setPoints2(rates(mutate) * sizeX2, rates(mutate) * sizeY2); 
        alive=true;
    }
    ...
    void getSecondAgentPred(const std::shared_ptr<agent> &b);
    ...
    void output_data(std::fstream &o, bool prey, int gen, int index) ;
     void outputfood(std::fstream &k, bool prey, int gen, int index) ;
